...
void EV3pp::outputSpeed(UI8 motors, int val) {
	if (val > 100) val = 100;
	if (val < -100) val = -100;
...
void EV3pp::outputPower(UI8 motors, int val) {
	if (val > 100) val = 100;
	if (val < -100) val = -100;
...
void EV3pp::outputStart(UI8 motors) {
...
void EV3pp::outputStop(UI8 motors, int type) {
...
deviceTypemode_t EV3pp::inputDeviceTypemode(UI8 port) {
...
UI8 EV3pp::inputFormat(UI8 port) {
...
void EV3pp::setSend(int(*_send)(void* data, int size)) {
	//this->_send = _send;
}
void EV3pp::setRecv(int(*_recv)(void* data, int max)) {
	//this->_recv = _recv;
}
...
EV3pp::EV3pp(int leftMotor, int rightMotor) {
	lMotor = leftMotor;
	rMotor = rightMotor;
	mnum = 0;
}

void EV3pp::connect(unsigned int comPort) {
	bool connected = 1;

	char comPortString[10];
	sprintf(comPortString, "%d", comPort);
	std::string fileName = "\\\\.\\COM" + std::string(comPortString);

	bluetoothHandle = CreateFile(fileName.c_str(),
...
void EV3pp::motors(int leftSpeed, int rightSpeed) {
...
EV3pp::~EV3pp() {
	...
	outputStop( EV3pp::MOTOR_A, STOP_FLOAT);
	outputStop( EV3pp::MOTOR_B, STOP_FLOAT);
	outputStop( EV3pp::MOTOR_C, STOP_FLOAT);
	outputStop( EV3pp::MOTOR_D, STOP_FLOAT);
...
int EV3pp::_send(void* data, int packSize) {
...
int EV3pp::_recv(void* data, int maxSize) {
